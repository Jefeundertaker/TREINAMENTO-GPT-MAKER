# -*- coding: utf-8 -*-
"""
Envia TODOS os PDFs localizados em <LOCAL_REPO>/PDFs para o GPT Maker.
...
# >>>>>>> ALTERAÇÃO SOLICITADA: usar apenas a pasta 'PDFs' do GitHub <<<<<<<
ROOT_SUBFOLDER = "PDFs"  # <- envia exclusivamente o que estiver em <repo>/PDFs
...
ap = argparse.ArgumentParser(description="Enviar PDFs da pasta 'PDFs' ao GPT Maker.")
...
def list_pdfs_in_pdfs_links() -> list[Path]:
    """
    Mesmo nome de função do script original, mas agora ela aponta para ROOT_SUBFOLDER = 'PDFs'.
    """
...
def main():
    ...
    if not pdfs:
        print("⚠️ Nenhum PDF encontrado em 'PDFs'.")
        return 0
