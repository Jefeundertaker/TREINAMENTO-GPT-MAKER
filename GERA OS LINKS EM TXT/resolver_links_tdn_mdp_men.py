...
# -*- coding: utf-8 -*-
"""
resolver_links_tdn_mdp_men.py
- Resolve links para MDP (DP/DPR/DMDP) e MEN (EN/ENR/DMEN/ENAPI/BOIN)
- Mantém hierarquia das linhas do txt
- Estratégia: REST (exato) -> REST (código/título) -> busca HTML
"""
...
SPACES_TRY = [None, "MDP", "MEN", "LDT", "LDS", "ENG", "DP", "EN"]
...
LOG = Path("debug_resolver_mdp_men.log")
...
def has_code(s: str) -> str | None:
    m = re.search(r"\b(DP\d{4}[A-Z]?|DPR\d{3}[A-Z]?|DMDP0*\d+|EN\d{4}[A-Z]?|ENR\d{3}[A-Z]?|DMEN0*\d+|ENAPI\d+|BOIN\d+)\b", s, re.I)
...
def is_section(line: str) -> bool:
    # heurística: linhas curtas sem código, ou com um único " - " que não pareçam item
    if has_code(line):
        return False
    if line.count(' - ') == 1 and not is_item(line):
        return True
    if len(line.split()) <= 7 and not any(line.startswith(p) for p in ITEM_PREFIXES):
        return True
    return False
...
def html_search(title: str):
    try:
        ...
        for a in soup.select("a[href]"):
            ...
            if len(txt) < 3:
                continue
            if title.lower() in txt.lower() or has_code(txt):
...
def main():
    in_path = Path("titulos_mdp_men.txt")
    if not in_path.exists():
        in_path = Path("titulos.txt")
    if not in_path.exists():
        print("Não encontrei 'titulos_mdp_men.txt' nem 'titulos.txt' na pasta atual.")
        sys.exit(1)
